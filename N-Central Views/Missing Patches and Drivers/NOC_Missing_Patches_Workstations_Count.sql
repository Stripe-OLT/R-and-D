SELECT        dbo.device.deviceid AS id, dbo.device.longname, dbo.device.deviceclass, COUNT(dbo.NOC_Missing_Patches_Workstations_tbl.updateid) AS MissingPatchesCount, 
                         ...
                         COUNT(CASE dbo.NOC_Missing_Patches_Workstations_tbl.Severity WHEN 'Unspecified' THEN 1 ELSE NULL END) AS MissingUnspecified, dbo.device.deleted, customer_parent.customerid, customer_parent.customername, 
...
GROUP BY dbo.device.deviceid, dbo.device.longname, dbo.device.deviceclass, dbo.device.deleted, customer_parent.customerid, customer_parent.customername, dbo.device.lastupdated